...
pub async fn check_oauth_token(credentials: Arc<types::Config>, credentials_location: String) {
    ...
    let res = req_client
        ...
        .query(&[
            ("client_id", &credentials.secret.client_id),
    ...
    let response = res
        .json::<OauthResponse>()
        .await
        .expect("You probably put wrong data in the config");
...
pub async fn get_title(
    ...
    let res = req_client
        ...
        .query(&[(
            "broadcaster_id",
            get_broadcaster_id(credentials).await.unwrap(),
        )])
        .send()
        .await
        .expect("Couldn't get a response");
    ...
    match response.data {
        ...
        None => Ok(format!(
            "Tried to get the title, but got an \"{}\" error",
...
pub async fn get_game(
    ...
    match response.data {
        Some(so) => {
            if so[0].game_name.is_empty() {
                Ok(format!(
                    "{} is in an empty category",
                    so[0].broadcaster_name
                ))
            } else {
                Ok(format!(
                    "{} is currently in the \"{}\" category",
                    so[0].broadcaster_name, so[0].game_name
                ))
            }
        }
